#%%
# API_KEY = "5xlEmwhfrjwPMvUAWjyjwfoncbMhJ30F"
...
# %%
import pandas as pd
df = pd.read_csv("../wgm_full_wave2_public_file_final (1)_csv.csv")
# %%
DATA_COMMONS_API_KEY = "je5aZwZ0J0wcP7yTCjpn6pGuvUUa91GQGEnd2rHjUqgA8tAK"
#%%
...
def getSeasonScheduleFrame(seasons,seasonType): 
    ...
    scheduleFrames =  [0]*4 #len(teamLookup['id'])
    ...
    for season in seasons:
        for i, id in enumerate(teamLookup['id']):
            print(i)
            time.sleep(1)
            # scheduleFrame = pd.concat([scheduleFrame,getRegularSeasonSchedule(season,id,seasonType)])
...
def getGameLogs(gameLogs,scheduleFrame):
    ...
    while i<len(scheduleFrame):
        ...
        time.sleep(1)
        gameLogs =  np.concatenate([gameLogs,getSingleGameMetrics(scheduleFrame.at[i,'GAME_ID'],scheduleFrame.at[i,'HOME_TEAM_ID'],
